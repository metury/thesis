...
pub fn read_file(filepath: &str) -> crate::lp::Instance {
	...
	let source: u32 = (&source_regex.expect("REASON").captures(&contents).unwrap()[1]).parse().unwrap();
	let capacity: u32 = (&capacity_regex.expect("REASON").captures(&contents).unwrap()[1]).parse().unwrap();
...
pub fn parse_solution(filepath: &str, g: &crate::lp::Graph) -> (DiGraph<f64, f64>, DiGraph<f64, f64>) {
